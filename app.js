...
// Values: Adā' = 2, Qaḍā' = 1, Jama'ath = 3, No = 0
// Rawatib: Yes = 1, No = 0
...
class GoogleSheetsAPI {
    constructor() {
        ...
        this.apiUrl = "https://script.google.com/macros/s/AKfycbyksk97sX5SjrtQXfY97I0VbE6mHDyKwVsdSOvq9TmQZ0g3GAEkLz4ae7WLeSJyJXUS/exec";
    }
...
async function submitPrayerForm(event) {
    ...
    try {
        ...
        console.log('Submitting prayer data:', {
            ...
            subh, zuhr, asr, magrib, isha,
            bSubh, bZuhr, aZuhr, bAsr, aMagrib, aIsha
        });
        ...
        if (result && result.success) {
            ...
            showSuccessAlertWithLogout('', fullSummary, function() {
                ...
                setTimeout(() => {
                    if (tempMsg.parentNode) tempMsg.remove();
                }, 5000);
            });
        } else {
...
// Helper function to convert numeric value to text status (UPDATED with Jama'ath)
function getPrayerStatusText(value) {
    switch(value) {
        case '2': return '✅ Adā\' (Prayed on time)';
        case '1': return '🕐 Qaḍā\' (Made up later)';
        case '3': return '🕌 Jama\'ath (Congregation)';
        case '0': return '❌ Not prayed';
...
console.log('%cFard Values: Adā\' = 2, Qaḍā\' = 1, Jama\'ath = 3, No = 0', 'color: #1f2937; font-size: 12px;');
